...
func (rq *RestaurantQuery) QueryAddress() *AddressQuery {
	query := &AddressQuery{config: rq.config}
...
func (rq *RestaurantQuery) WithAddress(opts ...func(*AddressQuery)) *RestaurantQuery {
	query := &AddressQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withAddress = query
	return rq
}
...
func (rq *RestaurantQuery) sqlAll(ctx context.Context) ([]*Restaurant, error) {
	...
	if query := rq.withAddress; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Restaurant)
		for i := range nodes {
			if fk := nodes[i].restaurant_address; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "restaurant_address" returned %v`, n.ID)
			}
			for i := range nodes {
