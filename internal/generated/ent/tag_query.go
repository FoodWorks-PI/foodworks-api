...
func (tq *TagQuery) QueryProduct() *ProductQuery {
...
func (tq *TagQuery) QueryRestaurant() *RestaurantQuery {
	query := &RestaurantQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TagQuery) WithProduct(opts ...func(*ProductQuery)) *TagQuery {
	query := &ProductQuery{config: tq.config}
...
func (tq *TagQuery) WithRestaurant(opts ...func(*RestaurantQuery)) *TagQuery {
	query := &RestaurantQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withRestaurant = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withProduct; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.ProductPrimaryKey[1], fks...))
			},
	...
	if query := tq.withRestaurant; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Tag, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.RestaurantPrimaryKey[1], fks...))
			},
